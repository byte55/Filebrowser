	...
	if(!empty($files))
	{
		...
		if(!empty($files['dir'])) 
		{
			...
			foreach($files['dir'] as $file)
			{
				...
				$json['html'] .= '<td><span><img class="icn" src="'.$imgs['controls']['folder_open'].'" alt="" /><span class="name">'.$file.'</span></span></td>';  // Name
				$json['html'] .= '<td><span class="size"></span></td>';  // Size
				$json['html'] .= '<td><span class="date">'.date('d.m.Y H:i',filemtime($dir.$file)).'</span></td>';  // change date
		...
		if(!empty($files['file']))
		{
			...
			foreach($files['file'] as $file)
			{
				...
				$json['html'] .= '<td><span class="size">'.bytesToSize(filesize($dir.$file)).'</span></td>';  // Size
				$json['html'] .= '<td><span class="date">'.date('d.m.Y H:i',filemtime($dir.$file)).'</span></td>';  // change date
...
else if($_POST['action'] == 'getMD5')
{
	if(file_exists(ROOT.$_POST['file_path']))
	{
		$json['md5'] = md5_file(ROOT.$_POST['file_path']);
	}
}
...
else if($_POST['action'] == 'getImage')
{
	$json['src'] = base64_encode_image(getRealPath($_POST['file_path']));
	//echo_array($_POST['file_path']);
}
if($json)
